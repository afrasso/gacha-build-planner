...
import {
  getArtifactMaxLevel,
  getInitialSubstatCountOdds,
  getMainStatOdds,
  getMainStats,
  getSubStatRollValues,
  getSubStats,
  getSubStatWeight,
} from "@/constants";
...
export const getInitialSubStatCount = ({ rarity }: { rarity: number }): number => {
  ...
  let current = 0;
  for (const [count, value] of Object.entries(getInitialSubstatCountOdds({ rarity }))) {
...
export const rollSubStats = ({ artifact }: { artifact: Artifact }): StatValue<Stat>[] => {
  ...
  const numRolls = Math.ceil((getArtifactMaxLevel({ rarity: artifact.rarity }) - artifact.level) / 4);
