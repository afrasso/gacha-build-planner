...
const SubStatsSelector: React.FC<SubStatsSelectorProps> = ({ mainStat, onUpdate, subStats = [] }) => {
  const [isAddingSubStat, setIsAddingSubStat] = useState(false);
  ...
  const cancel = () => {
    ...
    setValue(undefined);
    setIsValueValid(true);
    setIsAddingSubStat(false);
  };

  const validate = () => {
    const newIsStatValid = !!stat;
    setIsStatValid(newIsStatValid);
    const newIsValueValid = value != null && value >= 0 && value <= 1000;
      ...
      <div className="space-y-0">
        {subStats?.map((subStat) => (
          <div className="flex items-center space-x-2 py-1 rounded" key={subStat.stat}>
            ...
            <Button className="ml-auto" onClick={() => remove(subStat.stat)} size="sm" variant="ghost">
        ...
        <div className="flex items-center space-x-2 mt-2">
          {isAddingSubStat && (
            <>
              <Select onValueChange={onChangeStat} value={stat}>
